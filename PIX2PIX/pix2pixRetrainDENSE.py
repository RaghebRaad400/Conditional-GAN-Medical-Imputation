...
n_z_stat    = PARAMS.MC_samples      #set to 1
n_out       = PARAMS.sample_plots

#L1 Lambda used in pix2pix = 100
L1_LAMBDA=PARAMS.LAMBDA # put it as parameter
...
savedir = f"exps/Nsamples{n_train}_Ncritic{n_critic}_LAMBDA{L1_LAMBDA}_Etype{Etype}_ADDtanh{ADDtanh}_BS{batch_size}_Nepoch{n_epoch}_seed{PARAMS.seed_no}_GP{gp_coef}{save_suffix}"    # fixed the distinguisher, set zdim to 1
...
G_model = generatorpix2pix(Input_W=x_W,Input_H=x_H,Input_C=x_C,Z_Dim=z_dim,act_param=act_param,reg_param=reg_param,out_channels=16,layers=3)           #newpix2pix_like generator


D_model = critic(Input_W=x_W,Input_H=x_H,Input_C=x_C,k0=16,act_param=act_param,reg_param=reg_param,out_channels=16,layers=2)
...
stat_y  = tf.repeat(valid_y,n_z_stat,axis=0)   #what happens if the size is 1 of n_z_stat?
...
for i in range(start_,n_epoch):
    ...
    for true in train_data:
        ...
        z = glv(batch_size=batch_size)    #will not be used because bn doesnt use z is RESBLOCK
        ...
        if (n_iters) % (n_critic) == 0:
            ...
            with tf.GradientTape() as tape:
                ...
                #G_loss = -tf.reduce_mean(fake_XY_val)      #fix loss by adding L1 loss
                G_fake_loss = -tf.reduce_mean(fake_XY_val)
                MAE=tf.keras.losses.MeanAbsoluteError()
                L1=MAE(fake_X, true_X)*L1_LAMBDA
    ...
    if (i==0) or ((i+1) % PARAMS.evalmet_freq == 0):
        ...
        pred_ = G_model([stat_y,stat_z],training=None)  #nout*nzstat*128*128*4    check tf.repeat and what it does
...
#create a loop, with three columns: true Xs, mean of generated xs and std and 4 rows for each phase. 
    if (i==0) or ((i+1) % PARAMS.savefig_freq == 0):
        ...
        pred_ = G_model([stat_y,stat_z],training=None).numpy()  #nout*nzstat*128*128    check tf.repeat and what it does

        #ncol = 4
        #fig1,axs1 = plt.subplots(n_out, ncol, dpi=100, figsize=(ncol*5,n_out*5))  # pull inside
        #ax1 = axs1.flatten()
        #ax_ind = 0

        for t in range(n_out):   ####  TWO I?NDENTS  fix it
            ncol = 2
            fig1,axs1 = plt.subplots(4, ncol, dpi=100, figsize=(20,20))  # pull inside
            ax1 = axs1.flatten()
            ax_ind = 0
            ...
            #ax_ind +=1
                ...
                axs = ax1[ax_ind]
                pcm = axs.imshow(pred_[t,:,:,phase],aspect='equal',vmin=-1, vmax=1, cmap='pink')
                #if t==0:
