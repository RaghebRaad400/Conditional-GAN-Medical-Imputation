...
GANdir = f"exps/Nsamples{n_train}_Ncritic{n_critic}_Etype{Etype}_ADDtanh{ADDtanh}_Zdim{z_dim}_BS{batch_size}_seed{PARAMS.seed_no}_Nepoch{n_epoch}_GP{gp_coef}{save_suffix}"    # add seed 
...
G_model = generator(Input_W=x_W,Input_H=x_H,Input_C=x_C,Z_Dim=z_dim,act_param=act_param,reg_param=reg_param,out_channels=out_channels,layers=layers,ADDtanh=ADDtanh,Etype=Etype) 
...
#MSEcheckpoints=np.zeros((148,8))        # have a code that gives all MSE values then take file and work with it 
#SSIMcheckpoints=np.zeros((148,8))
#PSNRcheckpoints=np.zeros((148,8))
...
for checkpoint_id in range(50, 801, 50):
  print(checkpoint_id)
  K.clear_session()
  G_model = generator(Input_W=x_W,Input_H=x_H,Input_C=x_C,Z_Dim=z_dim,act_param=act_param,reg_param=reg_param,out_channels=out_channels,layers=layers,ADDtanh=ADDtanh,Etype=Etype)
...
np.save(savedir+'/MSE_{}.npy'.format(PARAMS.seed_no),MSEcheckpoints)
np.save(savedir+'/SSIM_{}.npy'.format(PARAMS.seed_no),SSIMcheckpoints)
np.save(savedir+'/PSNR_{}.npy'.format(PARAMS.seed_no),PSNRcheckpoints)
