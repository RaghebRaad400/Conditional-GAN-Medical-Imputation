...
#new one here
    ...
    pcm = axs.imshow(sample_mean[i,:,:,i%4],aspect='equal',vmin=-1, vmax=1, cmap='pink')
        #if t==0:
    axs.set_title(f'mean, min={round(np.min(sample_mean[i,:,:,i%4]),3)},max={round(np.max(sample_mean[i,:,:,i%4]),3)}',fontsize=30)
    divider = make_axes_locatable(axs)
    cax = divider.append_axes("right", size="5%", pad=0.05)                
    fig1.colorbar(pcm,ax=axs,cax=cax)
    axs.axis('off')
    ...
    axs = ax1[ax_ind]
    pcm = axs.imshow(sample_std[i,:,:,i%4],aspect='equal',vmin=0, cmap='pink')
        #if t==0:
    ...
    axs.axis('off')
    ax_ind +=1
    np.save(savedir+'/STD_{}_{}_{}.npy'.format(checkpoint_id,i,PARAMS.seed_no),sample_std[i,:,:,i%4])
